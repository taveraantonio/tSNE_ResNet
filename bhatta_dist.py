...
def bhatta_dist(X1, X2, method='continuous'):
    ...
    if method == 'noiseless':
        ...
        uX = np.unique(cX)
        A1 = len(X1) * (max(cX)-cX.min()) / len(uX)
        A2 = len(X2) * (max(cX)-cX.min()) / len(uX)
        bht = 0
        for x in uX:
            p1 = (X1==x).sum() / A1
            p2 = (X2==x).sum() / A2
            bht += sqrt(p1*p2) * (max(cX)-cX.min())/len(uX)

    elif method == 'hist':
        ###Bin the values into a hardcoded number of bins (This is sensitive to N_BINS)
        N_BINS = 10
        #Bin the values:
        h1 = np.histogram(X1,bins=N_BINS,range=(cX.min(),max(cX)), density=True)[0]
        h2 = np.histogram(X2,bins=N_BINS,range=(cX.min(),max(cX)), density=True)[0]
        #Calc coeff from bin densities:
        bht = 0
        for i in range(N_BINS):
            p1 = h1[i]
            p2 = h2[i]
            bht += sqrt(p1*p2) * (max(cX)-cX.min())/N_BINS
        ...
        for i in range(len(h1)):
            p1 = h1[i]
            p2 = h2[i]
            bht += sqrt(p1*p2) * (max(cX)-cX.min())/len(h1)
        ...
        d2 = get_density(X2)
        #Calc coeff:
        xs = np.linspace(cX.min(),cX.max(),N_STEPS)
        bht = 0
        for x in xs:
            p1 = d1(x)
            p2 = d2(x)
            bht += sqrt(p1*p2)*(cX.max()-cX.min())/N_STEPS
